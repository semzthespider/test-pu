...
class C_asisten_ppk extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model(array('M_asisten_ppk','user_model'));
		$p = $this->system_model->getMenuId('C_asisten_ppk');
		$this->d = $this->system_model->getPermission($this->session->role,$p);
		$this->pg = 'C_asisten_ppk';
	...
	public function index(){
		if($this->user_model->getSecure()) {
			if($this->d['r']==1){
				...
				$this->load->view('data_berkas/berkas2');
	...
	public function view($kd_pembebanan){
		if($this->user_model->getSecure()) {
			if($this->d['r']==1){
				$output['res'] = $this->M_asisten_ppk->list_unit($kd_pembebanan);
				$this->load->view('elemen/header',$output);
				$this->load->view('elemen/nav');
				$this->load->view('data_berkas/kirimberkas2');
				$this->load->view('elemen/footer');
			}else{
				redirect('home');	
			}
...
public function update()
		{
			if($this->user_model->getSecure()){
				if($this->d['w']==1){
					...
					$user_id3["n_date2"] 				= $this->input->post("n_date2");
					$user_id3["r_date3"] 				= $this->input->post("r_date3");
					$user_id3["n_date3"] 				= $this->input->post("n_date3");
					...
					$user_id2["idh3"] 					= $this->input->post("idh3");
					$user_id2["catatan"] 				= $this->input->post("catatan");
					$kd_pembebanan = $this->input->post('kd_pembebanan');
					$where = array('kd_pembebanan'=>$kd_pembebanan);
					$id_srt2 = $this->input->post('id_srt2');
					$where2 = array('id_srt2'=>$id_srt2);
					$this->M_asisten_ppk->updatedata('tbl_status2',$user_id1,$where2);
					$this->M_asisten_ppk->updatedata('tbl_berkas',$user_id2,$where);
					$this->M_asisten_ppk->updatedata('tbl_date',$user_id3,$where);
					redirect($this->pg);
				}else{
					redirect('C_asisten_ppk');	
				}
			}else{
				$this->login();
			}
