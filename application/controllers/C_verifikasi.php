...
class C_verifikasi extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('M_verifikasi','user_model'));
		$p = $this->system_model->getMenuId('C_verifikasi');
		$this->d = $this->system_model->getPermission($this->session->role,$p);
		$this->pg = 'C_verifikasi';
	...
	public function index(){
		if($this->user_model->getSecure()) {
			if($this->d['r']==1){
				...
				$this->load->view('data_berkas/berkas');
	...
	public function delete($kd_pembebanan)
	{
		if($this->user_model->getSecure()){
			if($this->d['d']==1){
				$this->berkas_model->delete_berkas($kd_pembebanan, $data);
				redirect($this->pg);
			}else{
				redirect('home');	
			}
	...
	public function view($id_user){
		if($this->user_model->getSecure()) {
			if($this->d['r']==1){
				$output['res'] = $this->M_verifikasi->geteditdata($id_user);
				$this->load->view('elemen/header',$output);
				$this->load->view('elemen/nav');
	...
	public function update()
		{
			if($this->user_model->getSecure()){
				if($this->d['w']==1){
					...
					$user_id3["n_date1"] 				= $this->input->post("n_date1");
					...
					$user_id2["idh2"] 					= $this->input->post("idh2");
					$user_id2["catatan"] 				= $this->input->post("catatan");
					$user_id3["r_date2"] 				= $this->input->post("r_date2");
					...
					$this->M_verifikasi->updatedata('tbl_berkas',$user_id2,$where);
					$this->M_verifikasi->updatedata('tbl_status1',$user_id1,$where);
					$this->M_verifikasi->updatedata('tbl_date',$user_id3,$where);
					redirect($this->pg);
				}else{
					redirect('C_verifikasi');	
				}
			}else{
				$this->login();
			}
