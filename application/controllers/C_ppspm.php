...
class C_ppspm extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model(array('M_ppspm','user_model'));
		$p = $this->system_model->getMenuId('C_ppspm');
	...
	public function index(){
		if($this->user_model->getSecure()) {
			if($this->d['r']==1){
				...
				$this->load->view('data_berkas/ppspm');
	...
	public function view($id_user){
		if($this->user_model->getSecure()) {
			if($this->d['r']==1){
				$output['res'] = $this->M_ppspm->list_unit($id_user);
				$this->load->view('elemen/header',$output);
				$this->load->view('elemen/nav');
				$this->load->view('data_berkas/kirimberkas4');
				$this->load->view('elemen/footer');
			}else{
				redirect('home');	
			}
...
public function update()
		{
			if($this->user_model->getSecure()){
				if($this->d['w']==1){
					...
					$this->M_ppspm->updatedata('tbl_status4',$user_id1,$where);
					$this->M_ppspm->updatedata('tbl_berkas',$user_id2,$where);
					$this->M_ppspm->updatedata('tbl_date',$user_id3,$where);
					redirect($this->pg);
				}else{
