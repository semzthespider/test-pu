...
class M_ppspm extends CI_Model {
   ...
   public function list_units($kd_pembebanan=null,$par=null,$args=null){
        $this->db->select('*');
        $this->db->from('tbl_berkas ');
        $this->db->join('tbl_status4 ','id_srt4=kd_pembebanan');
        if(!empty($kd_pembebanan)){ $this->db->where('kd_pembebanan',$kd_pembebanan); }
        $this->db->where('id_penerima',4);
        $this->db->order_by('kd_pembebanan', 'ASC');
        return $this->db->get();
    }

      public function list_unitsubdid($kd_pembebanan=null,$par=null,$args=null){
        $this->db->select('a.*');
        $this->db->from('tbl_berkas a');
        if(!empty($kd_pembebanan)){ $this->db->where('kd_pembebanan',$kd_pembebanan); }
        $this->db->where('idh5',5);
        $this->db->order_by('kd_pembebanan', 'ASC');
        return $this->db->get();
    }
    ...
    public function list_unit($kd_pembebanan=null){
        $this->db->select('*');
        $this->db->from('tbl_berkas ');
        if(!empty($kd_pembebanan)){ $this->db->where('tbl_status4.kd_pembebanan',$kd_pembebanan); }
        $this->db->join('tbl_status4','tbl_status4.kd_pembebanan=tbl_berkas.kd_pembebanan');
