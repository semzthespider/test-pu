...
class M_bendahara extends CI_Model {
   ...
   public function list_units($kd_pembebanan=null,$par=null,$args=null){
        ...
        $this->db->join('tbl_status5 ','id_srt5=kd_pembebanan');
        if(!empty($kd_pembebanan)){ $this->db->where('kd_pembebanan',$kd_pembebanan); }
        $this->db->where('id_penerima',5);
     ...
     public function list_unit($kd_pembebanan=null){
        $this->db->select('*');
        $this->db->from('tbl_berkas ');
        if(!empty($kd_pembebanan)){ $this->db->where('tbl_status5.kd_pembebanan',$kd_pembebanan); }
        $this->db->join('tbl_status5','tbl_status5.kd_pembebanan=tbl_berkas.kd_pembebanan');
