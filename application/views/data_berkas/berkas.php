                  ...
                  <div class="x_content">
                        ...
                        <thead>
                               <tr>
                                ...
                                <tbody>
                                <?php
                        $list_ = $this->M_verifikasi->list_unit();
                        $no = 1;
                        foreach($list_->result() as $row){
                          if($row->id_status==1){
                            ...
                            else if($row->jenisanggaran==56)
                            {
                              ...
                              {
                                $ta ='add jenis_anggaran';
                              }
                          $links = '<a href="'.base_url('C_verifikasi/view/'.$row->kd_pembebanan).'"class="btn btn-info ">'.$row->untuk_pembayaran.'<i ></i></a>';
                          echo '
                                <tr>
                                  <td>'.$no++.'</td>
                                  <td>'.$links.'</td>
                            ...
                            echo '<a href="'.base_url('C_verifikasi/edit-surat/'.$row->kd_pembebanan).'"  class="btn btn-primary ">Edit<i ></i></a> ';
                          if($permit['d']==1){
                          echo ' <a  href="'.base_url('C_verifikasi/delete/'.$row->kd_pembebanan).'" class="btn btn-danger alert_notif">Hapus<i ></i></a> ';
                          }
                          echo '</td>
                                </tr>';
                        }
                      ...
                      <!-- END EXAMPLE TABLE PORTLET-->
                        ...
                        <thead>
                               <tr>
                                ...
                                <tbody>
                                <?php
                        $list_ = $this->M_verifikasi->list_unit1();
                        $no = 1;
                        foreach($list_->result() as $row){
                           if($row->id_status==2){
                              $ac = 'Terkirim';
...
<!-- /.content -->
        ...
        jQuery(document).ready(function($){
            $('.alert_notif').on('click',function(){
                ...
                swal({
                title: "Hapus Data?",
                text: "Data Akan Terhapus Data Data berkas!",
                type: "warning",
